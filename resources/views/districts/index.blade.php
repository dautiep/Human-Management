...
@section('content')
				...
				<div class="row mb-2">
                    ...
                    <div class="col-sm-2">
						<div class="breadcrumb float-sm-right">
                            <button class="btn btn-primary" data-toggle="modal" data-target="#import-districts">Import file</button>
						</div>
					</div>
					<div class="col-sm-2">
						<div class="breadcrumb float-sm-right">
							<a class="btn btn-success" href="{{route('districts.create')}}">Thêm huyện</a>
						</div>
					</div>
					<div class="col-sm-2">
						...
						<table id="province-table" class="table table-bordered">
							<thead>
								<tr>
									...
									<th style="width: 150px;">Mã huyện</th>
							...
							<tbody>
								@foreach($districts as $district)
									<tr>
										<td>{{++$i}}</td>
										<td>{{$district->tinh->ten_tinh}}</td>
                                        <td>{{$district->ma_huyen}}</td>
										<td>{{$district->ten_huyen}}</td>
										<td>
											<a class="btn btn-warning" data-districtid={{$district->id}} 
                                                data-districttentinh="{{$district->tinh->ten_tinh}}"
												data-districtma="{{$district->ma_huyen}}"
												data-districtten="{{$district->ten_huyen}}"
												data-toggle="modal" data-target="#editDistrictModal">Edit
											</a>
											<button class="btn btn-danger" data-districtid={{$district->id}} 
												data-toggle="modal" data-target="#deleteDistrictModal">Delete
											</button>
										</td>
									</tr>
								@endforeach
    ...
    <!-- Start Modal Import -->
    <div class="modal modal-danger fade" id="import-districts" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title text-center" id="myModalLabel">Thông báo</h4>
                </div>
                <form action="{{ route('import-districts') }}" method="POST" enctype="multipart/form-data">
                {{csrf_field()}}
                    <input type="file" name="file" class="form-control">
                    <div class="modal-footer">
                        <button class="btn btn-success">Import Huyện</button>
                    </div>
                </form>
            </div>
        </div>
